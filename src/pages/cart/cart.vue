...
import { getMemberCartListAPI, deleteMemberCartAPI, putMemberCartBySkuIdAPI } from '@/services/cart'
...
/**
 * 异步获取会员购物车列表数据
 * 此函数通过调用 getMemberCartListAPI 来获取会员的购物车列表数据，并将其赋值给 cartList.value
 * 之所以需要这样的函数，是因为在前端页面中，通常需要展示会员的购物车信息，而这些信息需要从后端接口获取
 * 通过这种方式，可以确保购物车数据的实时性和准确性
 */
const getMemberCartListData = async () => {
  // 调用 getMemberCartListAPI 获取会员购物车列表数据
  const res = await getMemberCartListAPI()
  // 将获取到的购物车列表数据赋值给 cartList.value
 ...
 */
onShow(() => {
  // 如果会员信息仓库中有会员信息，则调用 getMemberCartListData 函数获取购物车列表数据
